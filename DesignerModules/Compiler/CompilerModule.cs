...
public class CompilerModule
{
    ...
    public bool CompileProject()
    {
        ...
        try
        {
            ...
            if (!GenerateSchedulesConfig(jsonPath))
            {
                EmitWarning("Schedules config skipped or failed.");
            }
            else
            {
                EmitMessage("Generated json/schedules.json");
            }
    ...
    private bool GenerateSchedulesConfig(string jsonPath)
    {
        if (_currentProject == null) return false;

        string schedulesPath = _currentProject.Paths.SchedulesPath;
        var schedulesArray = new JArray();

        // Try to load schedules.json from the schedules folder
        string schedulesFile = Path.Combine(schedulesPath, "schedules.json");
        if (File.Exists(schedulesFile))
        {
            try
            {
                var json = JObject.Parse(File.ReadAllText(schedulesFile));
                var schedulesArrayFromFile = json["schedules"] as JArray;
                
                if (schedulesArrayFromFile != null)
                {
                    // Copy all schedules from the source file
                    foreach (var schedule in schedulesArrayFromFile)
                    {
                        schedulesArray.Add(schedule);
                    }
                    EmitMessage($"Loaded {schedulesArray.Count} schedule(s) from schedules/schedules.json");
                }
            }
            catch (Exception ex)
            {
                EmitWarning($"Failed to load schedules.json: {ex.Message}");
                // Continue with empty array
            }
        }
        else
        {
            EmitMessage("No schedules.json found in schedules folder, creating empty schedules array");
        }

        var root = new JObject { ["schedules"] = schedulesArray };
        File.WriteAllText(Path.Combine(jsonPath, "schedules.json"), root.ToString(Newtonsoft.Json.Formatting.Indented));
        return true;
    }
    ...
    private void GenerateMinimalConfigs(string jsonPath)
    {
        ...
        // Note: schedules.json is now generated by GenerateSchedulesConfig() method
        // This fallback ensures it exists even if GenerateSchedulesConfig wasn't called
        var buildSchedulesPath = Path.Combine(jsonPath, "schedules.json");
        if (!File.Exists(buildSchedulesPath))
        {
